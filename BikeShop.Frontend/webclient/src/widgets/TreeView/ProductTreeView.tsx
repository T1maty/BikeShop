...
export default function ProductTreeView() {
    ...
    function createTree(parentId = 0) {
        ...
        let nodesToAdd:TreeViewData[] = [];

        data.map((n)=>{ if(n.parentId===parentId) nodesToAdd.push(n) })

        if(nodesToAdd.length>0)
        {
            return nodesToAdd?.map(n=>{
                return(
                    <TreeItem nodeId={n.id.toString()} label = {n.name}>{createTree(n.id)}</TreeItem>
                )
            })
        }
    }
