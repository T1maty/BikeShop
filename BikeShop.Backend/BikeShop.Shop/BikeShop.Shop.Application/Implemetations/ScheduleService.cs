...
namespace BikeShop.Shop.Application.Implemetations
{
    public class ScheduleService : IScheduleService
    {
        ...
        {
            if ((await _context.ScheduleItems.Where(n => n.ShopId == dto.ShopId).Where(n => n.TargetUser == dto.TargetUser).Where(n => n.TimeStart >= dto.Date.Date).Where(n => n.TimeFinish < dto.Date.Date.AddDays(1)).ToListAsync()).Count > 0) throw ScheduleErrors.ShiftAlreadyExist;
